...
while true; do
    ...
    case $yn in
		...
		echo "DISK_SPACE: 20 GB" >>${batch}                                                                    echo "MEMORY: 2500 MB" >> ${batch}
                echo "OS: centos7" >> ${batch}
                echo "CPU: 1" >> ${batch} ### hcana single core, setting CPU higher will lower priority
		echo "INPUT_FILES: ${tape_file}" >> ${batch}
		echo "COMMAND:/group/c-kaonlt/USERS/${USER}/hallc_replay_lt/UTIL_BATCH/Analysis_Scripts/DCCalib_Batch.sh ${runNum} ${SPEC} ${MAXEVENT}" >> ${batch} 
