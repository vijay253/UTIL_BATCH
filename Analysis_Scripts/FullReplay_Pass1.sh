...
if [ ! -f "$REPLAYPATH/ROOTfiles/coin_replay_Full_${RUNNUMBER}_50000.root" ]; then
    if [[ "${HOSTNAME}" != *"ifarm"* ]]; then
	eval "$REPLAYPATH/hcana -l -q \"SCRIPTS/COIN/PRODUCTION/FullReplay.C($RUNNUMBER,50000)\"" 
    elif [[ "${HOSTNAME}" == *"ifarm"* ]]; then
	eval "$REPLAYPATH/hcana -l -q \"SCRIPTS/COIN/PRODUCTION/FullReplay.C($RUNNUMBER,50000)\""| tee $REPLAYPATH/REPORT_OUTPUT/Full_replay_output_coin_production_${RUNNUMBER}_50000.report
    fi
else echo "50k replayfile already found for this run in $REPLAYPATH/ROOTfiles/ - Skipping replay step"
fi
